...
Then(
  "GET {string} returns status {int}",
  async function (this: AnywhenWorld, path: string, status: number) {
...
Then(
  ...
  function (this: AnywhenWorld, prefix: string) {
    const res = this.lastHttpResponse;
    if (!res) throw new Error("No prior response captured");
    const ct = res.headers.get("content-type") ?? "";
    expect(ct).toContain(prefix);
  },
);

Then("the JSON response has field {string}", async function (this: AnywhenWorld, field: string) {
  const res = this.lastHttpResponse;
  if (!res) throw new Error("No prior response captured");
