  ...
  return {
    ...
    async toggle(id: TaskId): Promise<Task> {
      return db.transaction().execute(async (trx) => {
        ...
        const now = new Date().toISOString();
        // todo→done sets completed_at; done→todo clears it. The CHECK
        // constraint on tasks.completed_at depends on both halves of this
        // contract — without the clear-on-untoggle, a re-toggled task
        // would carry a misleading stale timestamp.
